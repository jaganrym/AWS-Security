# Capstone Project: Securing JRT Cure's AWS Landing Zone

## Case Study Background

**JRT Cure** is a fictional 150-person digital health & wellness company. Their flagship product is a subscription app + telehealth platform: users book virtual consultations, store health records, track fitness data, and pay via a Stripe-tokenized card flow (so raw PAN never touches AWS, but the environment is still PCI-*adjacent*).

**Why security matters right now (the narrative driving this project):**
- JRT Cure just closed a Series B and signed its first enterprise wellness client, who is requiring a **SOC 2 Type II** report within 6 months.
...
**Objective:** Every account emits logs to one place, and JRT Cure can answer "what happened and when" for any resource.
